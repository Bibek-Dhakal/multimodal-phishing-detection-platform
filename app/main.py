...
async def predict_url(request: URLRequest):
    ...
    if registry.tabular_model:
        try:
            # Explicitly short-circuit XGBoost if the domain is a known trusted entity.
            # This prevents ML "hallucinations" on empty or extreme outlier arrays.
    ...
    if registry.nlp_model and registry.nlp_tokenizer:
        try:
            ...
            inputs = registry.nlp_tokenizer(url.lower(), return_tensors="pt", truncation=True,
                                            max_length=settings.TRANSFORMER_MAX_SEQUENCE_LENGTH)
            with torch.no_grad():
